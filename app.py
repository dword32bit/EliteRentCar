...
def pesanan():
    cursor.execute("""
        ...
        INNER JOIN accounts ON biodata.nik = accounts.nik ORDER BY cobapesanan.id DESC
    """)
...
def sewa1():
    if request.method == 'POST':
        ...
        if not (jenis_mobil and tanggal_pickup and jam_pickup and tanggal_dropoff and jam_dropoff):
            flash('Isi yang bener -_', 'add_boking_err')
            return redirect(url_for('user'))
            
        # Convert form data to datetime objects
        pickup_date = datetime.strptime(tanggal_pickup, '%Y-%m-%d')
        pickup_time = datetime.strptime(jam_pickup, '%H:%M')
        dropoff_date = datetime.strptime(tanggal_dropoff, '%Y-%m-%d')
        dropoff_time = datetime.strptime(jam_dropoff, '%H:%M')
        

        # Get current date and time
        current_date = datetime.now()

        if pickup_date.date() <= current_date.date() or dropoff_date.date() < pickup_date.date():
            flash('Yang Bener Aja Bang -_', 'add_boking_err')
            return redirect(url_for('user'))
    ...
    mobil_list = cursor.fetchall()
    cursor.close()
    return render_template('new/explore.html', mobil_list=mobil_list)
